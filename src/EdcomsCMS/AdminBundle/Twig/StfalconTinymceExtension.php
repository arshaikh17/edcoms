...
class StfalconTinymceExtension extends \Twig_Extension
{
    ...
    {
        return array(
            'edcoms_tinymce_init' => new \Twig_SimpleFunction(
                'edcoms_tinymce_init',
    ...
    public function tinymceInit($options = array())
    {
        ...
        $tinymceConfiguration = preg_replace(
            array(
                ...
                '/"paste_preprocess":"([^"]+)"\s*/',
                '/"video_template_callback":"([^"]+)"\s*/',
            ),
            array(
                'file_browser_callback:$1',
                'file_picker_callback:$1',
                '"paste_preprocess":$1',
                '"video_template_callback":$1',
            ),
            json_encode($config)
        );

        return $this->getService('templating')->render('EdcomsCMSAdminBundle:Script:tinymce/init.html.twig', array(
